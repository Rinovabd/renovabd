...
The v2 platform now exposes two audience-separated assistant channels. The public customer desk is available in the storefront UI and at `POST /api/assistant/customer`; it detects a bounded locale and intent, retrieves public D1 facts and published customer knowledge from the fresh `rinovabd-v2-assistant-knowledge` Vectorize index, asks the isolated Workers AI binding for a grounded JSON answer, validates the result, and returns the required **Direct answer / Why / Next step** shape with Worker-verified product cards. The authenticated customer session is required for own-order facts; no account or payment data is included in public answers.

The Studio copilot is available only after username/password sign-in at `POST /api/admin/assistant`. It accepts **only** a valid `x-admin-session`; the legacy bearer-token path is explicitly rejected for this channel. Staff chat is read-only and can propose an operation only with `requiresApproval: true`; existing Studio forms remain the only mutation path. Both channels persist a hash of the conversation identifier plus safe event metadata in D1, never raw message text.

The fresh API deployment includes Workers AI binding `AI` and Vectorize binding `ASSISTANT_KNOWLEDGE` pointing to `rinovabd-v2-assistant-knowledge`. The API deployment was accepted as `rinovabd-v2-api` revision `670c9e5d666e439db810bef4df5ba602`; the matching customer/Studio UI was accepted as `rinovabd-v2-web` revision `afd7601784c5476991168b3814d68a26`. Redacted live checks returned customer assistant `200`, staff unauthenticated `401` with `UNAUTHORISED`, and API health was checked separately after deployment. See [assistant architecture](docs/assistant-architecture.md) for the authorization matrix and grounded-response contract.
